...
function deleteProdList(productID, listID, productName) {
  ...
  document.getElementById("producto-eliminado").textContent =
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      if (response["success"]) {
        ...
        if (option == 2) {
          //Como se ha borrado, tenemos que cambiar la nueva lista seleccionada
          history.pushState(
            null,
            null,
            "wishList.php?listID=" + response["pos"]
          );
        }
        location.reload();
      } else {
