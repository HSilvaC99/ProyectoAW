...
<div class="container text-center shadow p-4 " id="main">
    <div class="row border-bottom border-2 border-primary">
        ...
        <div class="col-md-6 text-end mt-3 ">
            <a onmouseover="blueColor()" onmouseout="blackColorCL()" id="crear-lista" href="#" data-bs-toggle="modal"
                data-bs-target="#myModal" style="text-decoration: none; color: black;">Crear lista | </a>
            <a onmouseover="greenColorList()" onmouseout="blackColorCL()" id="edit-lista" href="#"
                data-bs-toggle="modal" data-bs-target="#myModalEdit" style="text-decoration: none; color: black;">Editar
                lista | </a>
            <a onmouseover="redColorList()" onmouseout="blackColorCL()" id="delete-lista" href="#"
                data-bs-toggle="modal" data-bs-target="#myModalDelet"
                style="text-decoration: none; color: black;">Eliminar lista</a>
        </div>
        <!-- MODAAAL-->
        <!-- Modal editar-->
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" id="crearListaBtn" data-bs-dismiss="modal"
                            onclick="createList(1,'<?= $nombreLista ?>',<?= $listaPred ?>)"
                            class="btn btn-primary">Editar lista</button>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="modal fade" id="myModalDelet" tabindex="-1" aria-labelledby="myModalLabelDel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button>
                        <button type="button" id="deletListaBtn" data-bs-dismiss="modal"
                            onclick="createList(2,'<?= $nombreLista ?>',<?= $listaPred ?>)"
                            class="btn btn-danger">Eliminar lista</button>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="alert alert-info" role="alert">
                            Usa las listas para guardar artículos que quieras comprar más adelante. Todas las listas son
                            privadas, a menos que las compartas con otros usuarios.
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button>
                        <button type="button" id="crearListaBtn" data-bs-dismiss="modal" onclick="createList(0)"
                            class="btn btn-primary">Crear lista</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="row " id="filas">
        <div class="col-md-3 border">
            ...
            <?php
                ...
                ?>
            <div id="lista-<?= $listas->getListID() ?>" class="mt-3 d-flex "
                style=" justify-content: space-between; background-color: #e6f2f5; height: 60px;"
                onmouseover="redColor(<?= $listas->getListID() ?>)"
                onmouseout="blackColor(<?= $listas->getListID() ?>)">
                <a href="wishList.php?listID=<?= $listas->getListID() ?>" class="ms-3 mt-2"
                    style="text-decoration: none; color: black;"><span style="text-align: left; font-size: 25px;"
                        id="link-<?= $listas->getListID() ?>"><?= $wishListDTO->getName() ?></span></a>
                <span class="me-3 mt-3" style="text-align: right;"
                    id="link-tipo-<?= $listas->getListID() ?>"><?= $tipo ?></span>
            </div>
            <?php
                    } else {
                        if (isset($_GET["listID"]))
                    ...
                    ?>
            <div id="lista-<?= $listas->getListID() ?>" class="mt-3 d-flex "
                style=" justify-content: space-between;  height: 60px;"
                onmouseover="redColor(<?= $listas->getListID() ?>)"
                onmouseout="blackColor(<?= $listas->getListID() ?>)">
                <a href="wishList.php?listID=<?= $listas->getListID() ?>" class="ms-3 mt-2"
                    style="text-decoration: none; color: black;"><span style="text-align: left; font-size: 25px;"
        ...
        <div class="col-md-9 border">
            ...
            <div id="eliminado" class="row mt-4 justify-content-start d-flex " style="display:none;">
                ...
                <div class="col-md-4 me-5 justify-content-start d-flex " id="p1" style="display:none;">
                    <span style="color:green ">
                        <i class="fas fa-check-circle text-success rounded-circle me-2" id="icon"
                            style="display:none;"></i>
                        <span style="display:none;" id="elim">Eliminado | </span>
                        <a onclick=" undo(<?= $listaPred ?>,<?= $producto->getID() ?>,<?= $prod->getDate() ?>)" href=""
                            style="display:none; " class="text-decoration-none" id="des">Deshacer</a>
                    </span>
                </div>
            ...
            <div class="row mt-4 " id="prod-<?= $producto->getID() ?>">
                ...
                <div class=" col-md-2 border">
                    <div class="img-fluid " id="product-img" style="width: 160px; height: 160px;"><a
                            href="<?php echo $url; ?>"><img class="img-fluid mt-2"
                                src="<?= $productsPath . $producto->getImgName(); ?>"></a></div>
                </div>

                <div class="order col-md-6 border border-secondary">
                    <div class="row">
                        <div class="col-md-12 justify-content-start d-flex a-truncate-cut fw-bold" aria-hidden="true"
                            style="height: 2.6em;">
                ...
                <div class="order col-md-4 border border-secondary">
                    ...
                    <div class="row">
                        ...
                        <div class="dropdown  col-md-6 mt-2">
                            <button class="btn btn-outline-secondary rounded-pill shadow border dropdown-toggle  w-100"
                                type="button" data-bs-toggle="dropdown" aria-expanded="false"
                                id="mover-<?= $producto->getID() ?>">
                                Mover
                            </button>
                            <ul class="dropdown-menu">
                                ...
                                <?php $wishListDTO = $wishListDAO->read($listas->getListID())[0];
                                                if ($wishListDTO->getID() != $listaPred) :
                                                ?>
                                <li><span class="dropdown-item" style="text-align: left;"
                                        id="mover-<?= $producto->getID() ?>"
                                        onclick="moveToSelectedList(<?= $wishListDTO->getID() ?>, <?= $listaPred ?>,<?= $producto->getID() ?>)"><?= $wishListDTO->getName() ?></span>
                                </li>
                        ...
                        <div class="col-md-6  mt-2">
                            <a class="btn btn-outline-secondary rounded-pill shadow col-md-6 border w-100"
                                id="delete-<?= $producto->getID() ?>"
                                onclick="deleteProdList(<?= $producto->getID() ?>, <?= $listaPred ?>,'<?= $producto->getName() ?>')">Eliminar</a>
